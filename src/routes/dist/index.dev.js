...
router.post('/signup', [check('firstName', 'First name required').exists(), check('lastName', 'Last name required').exists(), check('schoolId', 'School ID is required').exists(), check('email', 'Email required').exists().isEmail(), check('accountType', 'Account type required').exists(), check('clas', 'Class is required').optional(), check('password', 'Password required or is too short').exists().isLength({
  min: 6
})], controller.signup);
router.post('/login', [check('email', 'Email is required').exists().isEmail(), check('password', 'Password is required or is too short').exists().isLength({
  min: 6
})], controller.login);
